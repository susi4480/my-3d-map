...
las_dir = r"C:\Users\user\Documents\lab\data\suidoubasi\lidar"
output_path = r"C:\Users\user\Documents\lab\output_ply\suidoubasi_combined_classified_with_orange2.ply"

# === 分類パラメータ ===
voxel_size = 0.2
normal_wall_z_max = 4.5  # 壁として許容するZ上限
floor_z_max = 3.2        # Z > 4.0m を除外
horizontal_threshold = 0.95  # 法線Z > 0.9 を水平面と判定
...
# === ② LASファイルを一括読み込み・高さフィルタ ===
las_files = glob.glob(os.path.join(las_dir, "*.las"))
if len(las_files) == 0:
    raise FileNotFoundError(f"❌ LASファイルが見つかりません: {las_dir}")

lidar_all = []
for path in las_files:
    las = laspy.read(path)
    xyz = np.vstack((las.x, las.y, las.z)).T
    xyz = xyz[xyz[:, 2] <= floor_z_max]  # Z > 4.0m 除外
