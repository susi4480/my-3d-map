...
visual = np.zeros_like(free_bitmap, dtype=np.uint8)
for pt in green_pts:
    j = int((pt[1] - y_min) / GRID_RES)
    i = int((pt[2] - z_min) / GRID_RES)
    if 0 <= i < gh and 0 <= j < gw:
        visual[i, j] = 255
ax.imshow(visual, cmap='Greens', origin='lower')
ax.set_title("長方形の縁に配置された緑点")
