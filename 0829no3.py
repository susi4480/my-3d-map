...
# === 入出力 ===
input_floor_las = "/output/0910_floor_merged_raw.las"
input_lidar_las = "/output/0910_lidar_merged_raw.las"
output_final_las = "/output/0910_floor_lidar_morphfill_debug.las"

# === パラメータ ===
voxel_size = 0.2        # [m] 2Dグリッド解像度
z_upper_limit = 2.0     # [m] Z制限
morph_radius = 50       # [セル] Morphology補間カーネル半径（0.2m×50=10m）
search_radius_m = 12.0  # [m] KDTree探索半径（Morphology半径に近い値）
max_neighbors = 1000    # [点] KDTreeで探索する近傍点数
...
print(f"🗺️ グリッドサイズ: {grid.shape}, 占有セル数: {grid.sum():,}")
...
print(f"🔍 補間対象セル数: {new_mask.sum():,}")
new_ix, new_iy = np.where(new_mask)
new_xy = np.column_stack([new_ix*voxel_size + min_x,
                          new_iy*voxel_size + min_y])
...
if len(new_xy) > 0:
    ...
    for i in range(len(new_xy)):
        ...
        if neighbor_z.size > 0:
            new_z[i] = np.median(neighbor_z)

valid_points = ~np.isnan(new_z)
new_points = (np.column_stack([new_xy[valid_points], new_z[valid_points]])
              if np.any(valid_points) else np.empty((0, 3)))
print(f"✅ KDTreeで高さ付与できた補間点数: {len(new_points):,}")
print(f"⚠️ KDTreeで高さ付与できなかったセル数: {np.isnan(new_z).sum():,}")
