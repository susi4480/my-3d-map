...
- occupancy からM0の方式で「最大長方形」を抽出
  - 最初の長方形は無条件採用
  - 以降は最初の長方形に接しているもののみ採用
  - 長方形の縁セルを GREEN 点として抽出
- 全スライスのGREEN点を統合しLASに保存
"""
...
OUTPUT_LAS = r"/output/0908_M0onM5_voxel_style.las"
...
def find_max_rectangle(bitmap_bool: np.ndarray):
    """True=自由領域のbitmapから最大長方形を返す(top, left, h, w)"""
