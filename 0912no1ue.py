...
"""
【機能（M0 on M5 スタイル）】
- 入力LASをM5と同じくスライス化（中心線に沿って直線的に配置）
- 各スライスで v–z occupancy を作成
- occupancy からM0方式で「連結長方形領域の縁セル」を抽出
- 全スライスの縁点（GREEN）を統合し、LASに保存
"""
...
OUTPUT_LAS = r"/output/0912no1_M0onM5_merged_rects.las"
    ...
    if merged_bounds is not None:
        ...
        for zi in range(mt, mb):
            for yi in range(ml, mr):
                if not merged_mask[zi, yi]: continue
...
def main():
    ...
    for i in range(len(centers)-1):
        ...
        rect_edges_vz, _bbox = rectangles_on_slice_M0(points_vz, GRID_RES, MORPH_RADIUS, USE_ANCHOR_DOWNFILL, ANCHOR_Z, ANCHOR_TOL, MIN_RECT_SIZE)
        for vv,zz in rect_edges_vz:
            GREEN.append(vz_to_world_on_slice([vv,zz], c, n_hat))

    if not GREEN: raise RuntimeError("M0 on M5: 航行可能空間が見つかりません")
    write_green_las(OUTPUT_LAS, las.header, GREEN)
    print("✅ M0 on M5 完了")
    print(f" 中心線点数: {len(through)}")
    print(f" 断面数   : {len(centers)}")
    print(f" 出力点数 : {len(GREEN)}")
