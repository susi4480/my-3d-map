# -*- coding: utf-8 -*-
"""
【機能】スライスごとに長方形抽出（M0 on M5風, 緑色付き, スキップなし）
- 入力: /output/slices_m0style/*.las （スライス済みLASファイル群）
- 各スライスで v–z occupancy を作成
- occupancy からM0方式で「最大長方形＋直接接する長方形」を結合
- 外周セルを点群化し、slice_XXXX_rect.las として保存（緑点）
"""
...
# ===== 入出力 =====
INPUT_DIR  = "/output/slices_m0style"
OUTPUT_DIR = "/output/slices_m0style_rect"
...
def write_rect_las(path, header_src, pts_xyz):
    ...
    if {"red","green","blue"} <= set(las_out.point_format.dimension_names):
        ...
        las_out.green = np.full(N, 65535, dtype=np.uint16)  # 緑
...
def main():
    ...
    slice_files = sorted(glob(os.path.join(INPUT_DIR, "slice_*.las")))

    for f in slice_files:
        fname = os.path.basename(f).replace(".las", "_rect.las")
        out_path = os.path.join(OUTPUT_DIR, fname)

        las = laspy.read(f)
        X,Y,Z = np.asarray(las.x,float), np.asarray(las.y,float), np.asarray(las.z,float)
        if len(X) == 0: 
            pts_out = np.zeros((0,3))  # 空でもファイル作る
        else:
            # v–z に射影（Xをv、Zを高さとする仮処理）
            points_vz = np.column_stack([X, Z])
            rect_edges_vz = rectangles_on_slice_M0(points_vz, GRID_RES, MORPH_RADIUS,
                                                   USE_ANCHOR_DOWNFILL, ANCHOR_Z, ANCHOR_TOL, MIN_RECT_SIZE)
            pts_out = np.column_stack([
                np.full(len(rect_edges_vz), np.mean(X)) if len(rect_edges_vz)>0 else [],
                np.full(len(rect_edges_vz), np.mean(Y)) if len(rect_edges_vz)>0 else [],
                np.array(rect_edges_vz)[:,1] if len(rect_edges_vz)>0 else []
            ]) if len(rect_edges_vz)>0 else np.zeros((0,3))

        write_rect_las(out_path, las.header, pts_out)
