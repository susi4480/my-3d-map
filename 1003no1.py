# -*- coding: utf-8 -*-
"""
【機能】M0 on M5統合版（スライスごとに個別LAS出力, 面積・幅・Z下限フィルタ方式）
- 入力LASをM5方式で中心線に沿ったスライスを生成
- 各スライスで occupancy を構築し、M0方式で長方形抽出
- 面積・幅・Z下限フィルタでノイズや橋脚や川底深すぎる長方形を除去
...
OUTPUT_DIR = "/output/1003no2_7_3_filtered_slices"
...
Z_MIN_ALLOW   = -3.8  # 長方形の下端がこれ以下なら削除
...
def rectangles_on_slice_M0(points_vz, grid_res, morph_radius,
    ...
    """最大長方形を繰り返し抽出し、面積・幅・高さ(Z下限)フィルタで残す"""
    rects = []
    if len(points_vz) == 0:
        return []
    ...
    while np.any(free_work):
        ...
        if area >= MIN_RECT_AREA and width >= MIN_RECT_WIDTH:
            # Z下限フィルタ
            rect_z_min = z_min + top * grid_res
            if rect_z_min > Z_MIN_ALLOW:  # -3.8m以下は除外
