# -*- coding: utf-8 -*-
"""
【機能】PCA＋フォールバック版（スキップ防止＋安定4隅＋最緩連結＋航行空間抽出＋外側除去）
--------------------------------------------------------------------------------
...
OUTPUT_LAS_FINAL = "/workspace/output/1013no4_navspace_inneronly.las"
...
def main():
    ...
    for i in range(1, N-1):
        if not any(angle_turn_deg(series[c][i-1], series[c][i], series[c][i+1]) >= ANGLE_THRESH_DEG
                   for c in range(4) if not disabled[c][i]): continue
    ...
    print(f" 航行空間(灰): {len(filtered_pts):,}点 / 初期線(水): {len(bridge_initial):,}点 / 緑線: {len(bridge_pts):,}点")
