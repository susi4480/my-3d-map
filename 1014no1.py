# -*- coding: utf-8 -*-
"""
【機能】M0-on-M5統合版（GPU対応・下方向ダウンフィル補間＋Z制限＝スライス時点）
--------------------------------------------------------
- OpenCV(CUDA)＋CuPyを利用して Morphology/Downfill をGPU上で実行（CPU自動フォールバック）
- 中心線に沿ってスライスを生成（M5方式）
- 各スライスで occupancy 構築
- Morphology閉処理（固定半径25）＋下方向 Downfill（床側補完）
- スライス時点でZ上限（Z_MAX_FOR_NAV）を適用
- 最小矩形サイズ（MIN_RECT_SIZE=5セル＝0.5m角）を適用
- BFSで矩形を接続判定し、接している場合は結合
- 各スライスの縁点（緑点）をLAS出力
--------------------------------------------------------
依存:
...
OUTPUT_DIR = "/workspace/output/1014_downfill_floor_only"
...
def morph_close(grid_uint8, morph_radius):
    """Morph Close（GPU/CPU自動切替）"""
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2*morph_radius+1, 2*morph_radius+1))
    if gpu_available():
        g = cv2.cuda_GpuMat()
        g.upload(grid_uint8)
        out = cv2.cuda.morphologyEx(g, cv2.MORPH_CLOSE, kernel)
        return out.download()
    else:
        return cv2.morphologyEx(grid_uint8, cv2.MORPH_CLOSE, kernel)

def downfill_on_closed_gpu(closed_uint8, z_min, grid_res, anchor_z, tol):
    """
    GPU対応アンカーダウンフィル処理（下方向のみ）
    ----------------------------------------
    - anchor_z ± tol に点がある列を対象に
    - 下方向（床側）に補間して埋める
    """
    ...
    band = closed_bool[i_lo:i_hi+1, :].any(axis=0)
    for j in range(gw):
        if not band[j]:
            continue
        col = closed_bool[:, j]
        if not cp.any(col):
            continue
        imax = int(cp.max(cp.where(col)[0]))
        out[:imax+1, j] = True
    return (cp.asnumpy(out.astype(cp.uint8) * 255))

# ==== CPU処理補助 ====
def find_max_rectangle(bitmap_bool):
    """CPU上で最大長方形探索"""
...
def is_touching(a,b):
    tA,lA,hA,wA=a["top"],a["left"],a["h"],a["w"]
    tB,lB,hB,wB=b["top"],b["left"],b["h"],b["w"]
...
def rectangles_on_slice(points_vz, grid_res, morph_radius, use_anchor, anchor_z, anchor_tol):
    ...
    grid_raw=np.zeros((gh,gw),np.uint8)
    ...
    # Morphology閉処理＋Downfill（下方向のみ）
    closed0 = morph_close(grid_raw, morph_radius)
...
def write_green_las(path,header_src,pts_xyz):
    ...
    if {"red","green","blue"}<=set(las_out.point_format.dimension_names):
        las_out.red=np.zeros(N,np.uint16)
        las_out.green=np.full(N,65535,np.uint16)
        las_out.blue=np.zeros(N,np.uint16)
...
def main():
    ...
    for i in range(len(centers)-1):
        ...
        if len(rects_sorted)>=3:
            ...
            seed=[0] if mode=="merge" else [0,1,2]
        else:
            seed=[0]

        idxs=bfs_connected_indices(rects_sorted,seed)
