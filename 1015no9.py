# -*- coding: utf-8 -*-
"""
【機能】M0-on-M5統合版（MORPH_RADIUS=23 + ANCHOR_TOL=1.5 + 単独出力対応）
--------------------------------------------------------
- 中心線に沿ってスライスを生成（M5方式）
- Occupancy構築 → Morphology閉処理（半径23）＋Downfill（tol=1.5, imaxまで塗る＝「１」と同じ）
- Z上限を指定（Z<=Z_MAX_FOR_NAV のみ）
...
- 1〜2個しか矩形がなくてもスキップせず出力（seed={0}）
- half_len=40m（川方向に長く）, half_th=0.15m（法線方向に薄く）
...
OUTPUT_DIR = "/workspace/output/1015no9_final_refined"
...
def downfill_on_closed(closed_uint8, z_min, grid_res, anchor_z, tol):
    """アンカーダウンフィル処理（＝「１」と同じ：アンカー帯に実在があれば列の imax まで塗る）"""
...
def bfs_connected_indices(rects, seed_indices):
    """接触関係グラフ上で seed から到達可能な矩形インデックス集合（BFS）"""
    n = len(rects)
...
def main():
    ...
    # --- スライス処理 ---
    half_len=40.0    # 川方向に長く
    half_th = 0.15   # 法線方向に薄く
    for i in range(len(centers)-1):
        ...
        # ✅ 川の流れ方向に長く、法線方向に薄いスライス帯（＝「１」と同じ）
        m_band=(np.abs(u)<=half_len)&(np.abs(v)<=half_th)
        ...
        idxs = bfs_connected_indices(rects_sorted, seed_indices=seed)
